...
export const validationSchema = Joi.object({
  ...
  DATABASE_URL: Joi.string().when('NODE_ENV', {
    ...
    then: Joi.required().messages({
      'any.required': 'DATABASE_URL is required in production (should be injected from Azure Key Vault)',
    }),
  ...
  JWT_SECRET: Joi.string().when('NODE_ENV', {
    is: 'production',
    then: Joi.required().messages({
      'any.required': 'JWT_SECRET is required in production (should be injected from Azure Key Vault)',
    }),
