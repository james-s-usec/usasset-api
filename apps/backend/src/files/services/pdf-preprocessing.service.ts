...
export class PdfPreprocessingService {
  ...
  private async generateTilesForPageZoom(
    ...
    this.logger.log(`📊 Generated ${tilesGenerated} tiles for page ${page} zoom ${zoom}`);
   ...
   */
  public async tilesExist(fileId: string, page: number, zoom: number, x: number, y: number): Promise<boolean> {
    const tileBlobName = `tiles/${fileId}/${page}/${zoom}/${x}/${y}.png`;
   ...
   */
  private async uploadTileToBlob(blobName: string, buffer: Buffer): Promise<void> {
    try {
      // Access the blob client directly (we'll need to expose this)
      const blockBlobClient = (this.blobStorage as any).containerClient.getBlockBlobClient(blobName);
  ...
  public storeTileAsync(
    ...
    this.uploadTileToBlob(tileBlobName, tileBuffer).catch(error => {
      this.logger.error(`Background tile upload failed for ${tileBlobName}:`, error);
    });
  }
}